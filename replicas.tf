...
resource "google_sql_database_instance" "sql_database_instance_replica" {
  provider             = google-beta
  for_each             = var.create_database ? local.replicas : {}
  project              = var.project_id
  ...
  master_instance_name = google_sql_database_instance.sql_database_instance[0].name
